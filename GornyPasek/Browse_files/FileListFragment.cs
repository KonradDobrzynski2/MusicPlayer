    ...
    public class FileListFragment : ListFragment
    {
        ...
        public override void OnListItemClick(ListView l, View v, int position, long id)
        {
            ...
            if (fileSystemInfo.IsFile())
            {
                ...
                Toast.MakeText(Activity, "You selected file " + fileSystemInfo.Name, ToastLength.Short).Show();

                State.AppState._Extension_file = fileSystemInfo.Extension;
                State.AppState._FullName_file = fileSystemInfo.FullName;
                State.AppState._Name_file = fileSystemInfo.Name;

                string[] directory_table = fileSystemInfo.FullName.Split('/');
                int count = directory_table.Count();
                State.AppState._Folder_file = directory_table[count-2];

                int dlugosc_sciezki = fileSystemInfo.FullName.Length - fileSystemInfo.Name.Length;
                State.AppState._Path_file = fileSystemInfo.FullName.Remove(dlugosc_sciezki);

                List<string> name_file_list = new List<string>();

                List<string> paths_file = new List<string>();

                var directories_1 = Directory.EnumerateFiles(State.AppState._Path_file);

                foreach (var directory in directories_1)
                {
                    paths_file.Add(directory);
                    name_file_list.Add("  " + directory.Remove(0, dlugosc_sciezki));
                }

                State.AppState.paths_file_list = paths_file;

                State.AppState.name_file_list = name_file_list;
